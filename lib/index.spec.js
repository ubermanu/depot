...
test('remove all the values at once', () => {
  const items = depot()
  expect(items.add({}, {}, {}, {}, {}, {}, {}).empty().size()).toBe(0)
})

test('use a schema to enforce object structure', () => {
  const userSchema = {
    user: 'string',
    age: 'number|required',
    active: 'boolean'
  ...
  const items = depot(userSchema)

  const users = [
    { user: 'barney', age: 36, active: true },
    { user: 'fred', age: 40 },
    { user: 'pebbles', age: 1, active: '12' }
  ]

  items.add(users[0])
  expect(() => items.add(users[1])).toThrowError(/does not match/)
  expect(() => items.add(users[2])).toThrowError(/does not match/)
})

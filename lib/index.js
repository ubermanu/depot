...
export default function depot(schema = {}) {
  ...
  const self = {
    ...
    save() {
      ...
      let items = flattenDeep(arguments)
      validate(items, { id: 'required|string', ...schema })
  ...
  /**
   * Should throw an Error if an item does not match the passed schema.
   * Runs if the propTypes option is configured.
   *
   * @param {array|*} items
   * @param {object} validationSchema
   */
  function validate(items, validationSchema) {
    ...
    for (let item of items) {
      if (isObject(item)) {
        let v = validator.validate(item, validationSchema)
        if (v.hasError) {
          throw new Error(`The data does not match the given schema: ${JSON.stringify(v.errors)}`)
        }
      }
    }
  }

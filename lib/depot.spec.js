...
test('use a custom uuid function', () => {
  const items = depot({ uuid: () => '01234' })
  expect(items.add({}).first().id).toBe('01234')
})

test('use a custom primary key', () => {
  const items = depot({ id: '__id' })
  expect(items.add({}).first()).toHaveProperty('__id')
})
